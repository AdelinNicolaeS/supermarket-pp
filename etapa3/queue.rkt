...
(define (queue-empty? q)
  (and (equal? 0 (queue-size-l q)) (equal? 0 (queue-size-r q))))
...
(define (dequeue q)
  (if (null? (queue-left q))
      (struct-copy queue q [left (cdr (reverse (queue-right q)))] [right null] [size-l (- (queue-size-r q) 1)] [size-r 0])
